/* eslint-disable spaced-comment,no-delete-const,no-use-before-define,no-delete-var */
/*
  「旧版IE淘汰行动」之「旧版IE升级提示弹窗代码」
   https://support.dmeng.net/ie-alert-warning.html
*/
// This file need transpile for production

const _iealwn = _iealwn || []
...
_iealwn.browser = function () {
  ...
  if (ua.indexOf('QQBrowser') > -1) {
    ...
    browser = 'UC'
  } else if (ua.indexOf('LBBROWSER') > -1 || (!!window.external && !!window.external.LiebaoAutoFill_CopyToClipboard)) {
    browser = 'LBBROWSER'
  } else if (ua.indexOf('Maxthon') > -1 || (!!window._MX$MF_6a39a14b_c884_4333_a26b_08330f10ab4aMxBIG)) {
    browser = 'Maxthon'
  } else if (ua.indexOf('360SE') > -1) {
    browser = '360SE'
  } else {
    // IE9、10
    if (ua.match(/MSIE (9|10)/g)) {
      // 根据窗口标题栏、网址栏、收藏栏高度判断
...
_iealwn.close = function () {
  const el = document.getElementById('_iealwn_div')
  el.outerHTML = ''
  delete el
}
...
#iealertwn .iealwn-box{width:850px;margin:0 auto 20px;overflow:hidden;border:1px solid #ededed}
#iealertwn .iealwn-main{border:10px solid #fff}
...
#iealertwn .iealwn-notice p{margin:.5em 0;padding:2px}
#iealertwn a{color:#da2128;text-decoration:underline}
#iealwn-browsers .iealwn-browser-link{text-decoration:none;display:inline-block;width:110px;color:#c4691f}
...
img.iealwn-guide{background:url(${_iealwn.dir}/images/blank.png) no-repeat center center #e9ecef}`
  ...
  const browsers = [
    {
      ...
      'durl': 'https://www.google.cn/chrome/?hl=zh-CN&standalone=1'
    }, {
      'slug': 'edge',
      'name': '微软浏览器',
      'url': 'https://www.microsoft.com/windows/microsoft-edge',
      'durl': 'https://www.microsoft.com/windows/microsoft-edge'
    }, {
      'slug': 'qqbrowser',
      'name': 'QQ浏览器',
      'url': 'http://browser.qq.com',
      'durl': 'http://browser.qq.com'
    }, {
      'slug': 'se360',
      'name': '360安全浏览器',
      'url': 'https://browser.360.cn/se/',
      'durl': 'https://browser.360.cn/se/'
    }
  ]

  const isXP = navigator.userAgent.indexOf('Windows NT 5') !== -1
  const is7or8 = navigator.userAgent.indexOf('Windows NT 6') !== -1
  let browsersHtml = ''
  for (let i = 0; i < browsers.length; i++) {
    const slug = browsers[i]['slug']

    let classes = 'iealwn-browser iealwn-browser_' + slug
    ...
    if (isXP && slug.match(/(chrome|firefox|edge)/g)) {
      ...
      let warningTitle = browsers[i]['name'] + '已经全面停止支持 Windows XP 系统！请选择其他浏览器。'
      if (slug === 'edge') { warningTitle = 'Microsoft Edge 专为 Windows 10 打造！请选择其他浏览器。' }
  ...
  let currentBrowserHtml = ''
  currentBrowserHtml += '<p>您使用的不是 IE 浏览器？是 '
  let i = 1
  let count = 0
  for (const prop in _iealwn.browserNames) {
    count++
  }
  for (const browser in _iealwn.browserNames) {
  ...
  currentBrowserHtml += ' 其中一个吗？若是如此，您暂可不必升级，点击名称查看教程并按步骤切换至极速内核也可正常访问。</p>'

  const alertHtml =
    `<div id="iealertwn"><div class="iealwn-wrap"><div class="iealwn-box"><div class="iealwn-main">
      <div class="iealwn-line">
        <a href="https://support.dmeng.net/ie-alert-warning.html" target="_blank">旧版 Internet Explorer 浏览器升级弹窗</a>
      </div>
      <div class="iealwn-notice">
        <div class="iealwn-title iealwn-title_alert">危险！您正在使用的 IE${_iealwn.iever ? _iealwn.iever : ''} 浏览器已过期，请立即升级！</div>
        <p>自2016年1月12日起，微软不再为 IE11 以下版本提供相应支持和更新。没有关键的浏览器安全更新，您的电脑可能易受有害病毒、间谍软件和其他恶意软件的攻击，它们可以窃取或损害您的业务数据和信息。为确保您的电脑安全，请停止使用IE的过期版本！</p>
        <div id="_iealwn_browserAlert">
          <div class="iealwn-title iealwn-title_horn">是时候升级你的浏览器了！</div>
          <p>如您坚持使用当前浏览器访问本站，你将看到排版错误、功能不全、无法正常使用的网页，甚至是空白或乱码。请至少升级至 IE${_iealwn.outver + 1} 或使用更先进的浏览器再访问。</p>
        </div>
        <div id="_iealwn_browserAlert_new">${currentBrowserHtml}</div>
        <div class="iealwn-title iealwn-title_browser">下载更先进的浏览器</div>
        <div id="iealwn-browsers">${browsersHtml}<div class="iealwn-browser-clear"></div>
        </div>
        <div class="iealwn-title iealwn-title_faq">为什么会出现这个弹窗？</div>
        <p>如果你不知道升级浏览器是什么意思，请请教一些熟悉电脑操作的朋友。</p>
      </div>
      <div class="iealwn-line">
...
_iealwn.browserAlert = function (browser) {
  ...
  const names = _iealwn.browserNames
  let alertHtml = ''

  if (names[browser]) {
    const browserName = names[browser]

    alertHtml +=
      `<div class="iealwn-title iealwn-title_horn">您使用的是${browserName}吗？</div>
        <p>您正在使用的可能是${browserName} IE 兼容模式。若是如此，您暂可不必升级，按下图所示步骤切换至极速内核也可正常访问。（示意图仅供参考，浏览器不同版本可能有差异）</p>
        <p><img class="iealwn-guide" src="${_iealwn.dir}/images/switch-${browser}.png" width="586" height="${_iealwn.imgHeight[browser]}" alt="示意图" title="如图片未显示，请点击右键选择 显示图片(H)"></p>
      `
    alertHtml += '<p>其他双核浏览器切换极速内核示意图（点击名称查看）：'
...
if (_jsver === 0) {
  ...
  if (_iealwn.iever <= _iealwn.outver) {
    _iealwn.show = true
    if (_iealwn.show) {
      const s = document.getElementById('_iealwn_js')
      const path = s.src.split('?')[0]
      _iealwn.dir = path.split('/').slice(0, -1).join('/')
      /* cdn */_iealwn.dir = 'https://static.support.dmeng.net/ie-alert-warning'
